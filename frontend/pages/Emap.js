...
import { Text, StyleSheet, Dimensions, View, Alert, Image } from 'react-native';
...
export default class Emap extends React.Component {
  ...
  getLatLng = async (start, end) => {
    ...
    try {
      axios
        .get(
          `https://naveropenapi.apigw.ntruss.com/map-geocode/v2/geocode?query=${start}`,
          {
            headers: {
        ...
        .then((response) => {
          this.setState({
            start_lat: response.data.addresses[0].y,
            start_lng: response.data.addresses[0].x,
          });
        });
    } catch (error) {
    ...
    try {
      axios
        .get(
          `https://naveropenapi.apigw.ntruss.com/map-geocode/v2/geocode?query=${end}`,
          {
            headers: {
              'Access-Control-Allow-Origin': '*',
              'X-NCP-APIGW-API-KEY-ID': ID_KEY,
        ...
        .then((response) => {
          this.setState({
            end_lat: response.data.addresses[0].y,
            end_lng: response.data.addresses[0].x
          });
        })
        .then(() => {
          ...
          <View style={styles.view}>
            ...
            >
              <Marker coordinate={{ latitude: cur_lat, longitude: cur_lng }}>
                <Image
                  source={require('../components/marker_img.png')}
